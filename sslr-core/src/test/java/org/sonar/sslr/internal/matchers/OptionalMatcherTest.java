...
public class OptionalMatcherTest {
  ...
  private OptionalMatcher matcher;

  @Before
  public void setUp() {
    subMatcher = mock(Matcher.class);
    subContext = mock(MatcherContext.class);
    matcher = new OptionalMatcher(subMatcher);
  ...
  @Test
  public void test() {
    when(subContext.runMatcher()).thenReturn(true).thenReturn(false);
    assertThat(matcher.match(context)).isTrue();
    verify(subContext).runMatcher();
    assertThat(matcher.match(context)).isTrue();
    verify(subContext, times(2)).runMatcher();
  }
