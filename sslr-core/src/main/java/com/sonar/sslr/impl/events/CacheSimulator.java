...
public final class CacheSimulator extends ParsingEventListener {
  ...
  private final int maximalCacheSize = 50;
  ...
  private int getFirstHit(HashMap<Matcher, HashSet<Integer>> cacheSet, int cacheSizeUpperBound, Matcher matcher, List<Integer> cache,
      int lexerIndex) {
    if (cacheSizeUpperBound <= 0) {
      return -1;
    }
    ...
    while (reverseIterator.hasPrevious()) {
      ...
      if ( !distinctCacheEntries.add(cachedIndex)) {
        continue;
      }
      cacheSize++;
  ...
  private int lookupInCaches(HashMap<Matcher, HashSet<Integer>> cacheSet, int cacheSizeUpperBound, Matcher matcher, List<Integer> cache,
      int lexerIndex, HashMap<Integer, Integer> hits, HashMap<Integer, Integer> misses) {
  ...
  private void lookup(Matcher matcher, int lexerIndex) {
    ...
    cacheSizeUpperBoundStackPositive.push(lookupInCaches(matchesSet, cacheSizeUpperBoundStackPositive.peek(), matcher,
        matchesCache.get(matcher), lexerIndex, matchesHits, matchesMisses));
    cacheSizeUpperBoundStackNegative.push(lookupInCaches(mismatchesSet, cacheSizeUpperBoundStackNegative.peek(), matcher,
        mismatchesCache.get(matcher), lexerIndex, mismatchesHits, mismatchesMisses));
  ...
  public void endParse() {
    ...
    for (int i = 1; i <= maximalCacheSize + 1; i++) {
      System.out.println("\t" + (i == maximalCacheSize + 1 ? "Full" : String.format("%4d", i)) + ": "
          + String.format("%10d", matchesHits.get(i)) + " matches hits, " + String.format("%10d", matchesMisses.get(i))
          + " matches misses, " + String.format("%10d", matchesHits.get(i) + matchesMisses.get(i)) + " total, "
          + String.format("%10d", mismatchesHits.get(i)) + " mismatches hits, " + String.format("%10d", mismatchesMisses.get(i))
          + " mismatches misses, " + String.format("%10d", mismatchesHits.get(i) + mismatchesMisses.get(i)) + " total, "
          + String.format("%10d", bothHits.get(i)) + " both hits, " + String.format("%10d", bothMisses.get(i)) + " both misses, "
          + String.format("%10d", bothHits.get(i) + bothMisses.get(i)) + " total");
    }
    System.out.println("Current memoizer: " + currentMemoizerHits + " hits, " + currentMemoizerMisses
        + " misses, total visited matchers = " + (currentMemoizerHits + currentMemoizerMisses));
  }
