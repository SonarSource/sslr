...
public final class ProfilerStream {
	...
	public static void print(Profiler profiler, PrintStream stream) {
		List<Map.Entry<RuleMatcher, RuleCounter>> sortedRules = asSortedList(profiler.ruleStats.entrySet(),
				new Comparator<Map.Entry<RuleMatcher, RuleCounter>>() {
					public int compare(Entry<RuleMatcher, RuleCounter> o1, Entry<RuleMatcher, RuleCounter> o2) {
						if (o1.getValue().getTotalNonMemoizedHitCpuTime() == o2.getValue().getTotalNonMemoizedHitCpuTime()) return 0;
		...
		stream.println("How many backtracks: " + profiler.getBacktracks() + " (time avg: " + nsToMs(profiler.getAverageBacktrackCpuTime()) + "ms, max: " + nsToMs(profiler.getMaxBacktrackCpuTime()) + "ms, lookahead avg: " + String.format("%.2f", profiler.getAverageLookahead()) + ", max: " + profiler.getMaxLookahead() + ")");
