...
public class OfTypeAstQuery extends AstQuery {
  ...
  public OfTypeAstQuery(AstNodeType type1, AstNodeType... remainingTypes) {
    this(null, type1, remainingTypes);
  }

  public OfTypeAstQuery(AstQuery parent, AstNodeType type1, AstNodeType... remainingTypes) {
    super(parent);
    this.types = (Set) ImmutableSet.builder().add(type1).addAll(Arrays.asList(remainingTypes)).build();
  }

  @Override
  public AstNode next(AstNode node) {
    AstNode current = super.next(node);
    while (current != null && !types.contains(current.getType())) {
      current = super.next(current);
    }
    return current;
  }
